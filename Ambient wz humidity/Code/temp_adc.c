...
unsigned short read_temp(){
  ...
  unsigned short digital_value;
   ...
   AD0CR_bit.SEL = 2;    //Conversion to be done on selected channel
   ...
   //digital_value=AD0DR1;
   
   digital_value = AD0GDR_bit.RESULT;  //read the 10-bit digital conversion result
   
   //convert the digital level read from the sensor
   //to match the actual value displayed on the sensor
   //in the range of 15c-35c
   //beacuase the output voltage changes depending on the supply 
   //voltage so its characteristic curve for temp in the range of
   //15-35 with suply voltage of 5v is interms of the digital value 
   //read in is 
   //      digital value =3*actual_temp+2 at 15c and
   //      digital value =3*actual_temp+4 at 35c
   // which results in tolerable error of 2/3=0.66 above 30c
   digital_value=digital_value-2;
   digital_value/=3;
